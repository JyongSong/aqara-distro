...
#
#   bash scripts/backup-db.sh              # DB + Storage 백업
#   bash scripts/backup-db.sh --db-only    # Storage 건너뛰기
#   BACKUP_ROOT=/Volumes/ext/aqara-distro bash scripts/backup-db.sh   # 다른 위치로
#   KEEP_DAILY=30 bash scripts/backup-db.sh                           # 보관 개수 변경
#
# 필요한 것: pg_dump 17 이상 (brew install libpq), .env.local 의 DIRECT_URL
set -euo pipefail

PROJECT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
BACKUP_ROOT="${BACKUP_ROOT:-$HOME/data_backup/aqara-distro}"
ENV_FILE="${ENV_FILE:-$PROJECT_DIR/.env.local}"
KEEP_DAILY="${KEEP_DAILY:-14}"       # 최근 몇 회분을 남길지. 매월 1일 백업은 이와 무관하게 보존한다
MIN_PG_MAJOR="${MIN_PG_MAJOR:-17}"   # 서버 select version() 의 메이저 버전
DB_ONLY=0
[[ "${1:-}" == "--db-only" ]] && DB_ONLY=1
...
# 스냅샷 전체 크기 — hard link 로 이어진 파일도 포함해서 센다.
...
# --- pg_dump 찾기 (서버 메이저 버전 이상이어야 한다) --------------------------
find_pg_dump() {
  for candidate in \
    "${PG_DUMP:-}" \
    "/opt/homebrew/opt/postgresql@$MIN_PG_MAJOR/bin/pg_dump" \
  ...
  do
    ...
    if (( major >= MIN_PG_MAJOR )); then echo "$candidate"; return 0; fi
  done
  return 1
}
PG_DUMP_BIN="$(find_pg_dump)" || die "pg_dump $MIN_PG_MAJOR+ 가 없다. 먼저: brew install libpq"
...
[[ -n "$DB_URL" ]] || die "$ENV_FILE 에 DIRECT_URL 이 없다 (Supabase 대시보드 > Connect > Session pooler)"
[[ "$DB_URL" == *":6543"* ]] && die "DIRECT_URL 이 트랜잭션 풀러(6543)를 가리킨다. 5432 세션 포트가 필요하다"

STAMP="$(date '+%Y-%m-%d_%H%M')"
DEST="$BACKUP_ROOT/$STAMP"

# 직전 백업의 storage 디렉터리 — 파일을 다시 받지 않고 hard link 로 잇는 대상이다.
PREV_STORAGE=""
...
mkdir -p "$DEST"
chmod 700 "$BACKUP_ROOT" "$DEST"   # 개인정보(거래처 담당자 연락처 등)가 들어간다
...
# --- 4. Storage 버킷 (product-images) -----------------------------------------
if (( DB_ONLY == 0 )); then
  if [[ -n "$PREV_STORAGE" ]]; then
    log "Storage 버킷 확인 중 (새 파일만 내려받고, 기존 파일은 $(basename "$(dirname "$PREV_STORAGE")") 에 hard link)…"
  else
